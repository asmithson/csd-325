...
LAKE = '~' # Character for the lake
TREE = 'A'
FIRE = '@'
EMPTY = ' '

# (!) Try changing these settings to anything between 0.0 and 1.0:
INITIAL_TREE_DENSITY = 0.10  # Amount of forest that starts with trees.
GROW_CHANCE = 0.07  # Chance a blank space turns into a tree.
...
def createNewForest():
    ...
    lake_radius = min(WIDTH, HEIGHT) // 4 # Radius of the lake
    
    for x in range(WIDTH):
        for y in range(HEIGHT):
            # Check if the current space is unoccupied within the lake radius
            if ((x - lake_center_x) ** 2 + (y - lake_center_y) ** 2) ** 0.5 <= lake_radius: # Calculating center of the lake & use for the radius
                forest[(x, y)] = LAKE
